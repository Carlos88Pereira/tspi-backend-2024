...
var numeroDeOcorrencias = //contarLetra(frase, letra);
  console.log(
    "Número de vezes que a letra '" + letra + "' ocorre na frase:",
    numeroDeOcorrencias
  );
...
function calcularTempoDeTrabalho(horaEntrada, horaSaida) {
  ...
  entrada.setHours(
    parseInt(partesEntrada[0]),
    parseInt(partesEntrada[1]),
    parseInt(partesEntrada[2])
  );

  var saida = new Date();
  var partesSaida = horaSaida.split(":");
  saida.setHours(
    parseInt(partesSaida[0]),
    parseInt(partesSaida[1]),
    parseInt(partesSaida[2])
  );
  ...
  if (entrada < horaAbertura || saida > horaFechamento) {
    console.log(
      "Os horários de entrada e saída devem estar dentro do intervalo de trabalho (08:00:00 - 18:00:00)."
    );
    return;
  }
...
console.log(
  "A nota mais dalta é " + notaMaisAlta + ", e pertence a " + AlunoMaisAlta
);
...
console.log(
  "A nota mais baixa é " + notaBaixa + ", e pertence a " + alunoMaisBaixo
);
...
function closestAverage(alunos) {
  var avg = notaMedia(alunos);
  var mindiff = Number.POSITIVE_INFINITY;
  var closestAluno = null;
  for (let i = 1; i < alunos.length; i++) {
    var diff = Math.abs(avg - alunos[i].grade);
    if (diff < mindiff) {
      mindiff = diff;
      closestAluno = alunos[i];
    }
  }
  return closestAluno;
  console.log("O aluno com a nota mais aproximado à méd");
}
closestAverage(alunos);
...
var countN = negativas(alunos);
console.log("Existe" + countN + "Negativas.");
...
function calculateTime(he, me, se, hs, ms, ss) {
  ...
  var horasReminder = diff % 3600;
  var horas = (diff - horasReminder) / 3600;
  var sec = horasReminder % 60;
  var min = (horasReminder - sec) / 60;

  console.log("O Funcionario trabalhou:" + horas + ":" + min + ":" + sec);
}
calculateTime(8, 0, 0, 9, 20, 12);


//invocação, e chamar a função (ver as definiçoes)
